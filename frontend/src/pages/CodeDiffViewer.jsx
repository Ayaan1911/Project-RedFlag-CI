...
function AutoFixButton({ autoFixPrUrl }) {
  return (
    <button
      style={{
        background: autoFixPrUrl ? '#E63946' : '#374151',
        color: '#fff',
        border: 'none',
        padding: '10px 24px',
        borderRadius: '8px',
        cursor: autoFixPrUrl ? 'pointer' : 'not-allowed',
        fontFamily: 'monospace',
        fontSize: '13px',
        fontWeight: 'bold',
        opacity: autoFixPrUrl ? 1 : 0.6,
      }}
      onClick={() => autoFixPrUrl && window.open(autoFixPrUrl, '_blank')}
      disabled={!autoFixPrUrl}
    >
      View Auto-Fix PR →
    </button>
  )
...
export default function CodeDiffViewer() {
  ...
  useEffect(() => {
    async function loadScan() {
      ...
      setScanData(data)
      setFinding(data.findings[parseInt(findingIndex, 10)])
      setLoading(false)
    }
  ...
  const autoFixPrUrl = scanData?.auto_fix_pr_url
  ...
  return (
    <div style={{ background: '#0D0F14', minHeight: '100vh', padding: '24px' }}>
        ...
        {finding.compliance_violations?.length > 0 && (
          <div style={{
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '10px' }}>
              {finding.compliance_violations.map((violation) => (
                <span key={violation} style={{
                  background: '#1e3a5f',
                  color: '#93C5FD',
      ...
      <div style={{ display: 'flex', gap: '12px' }}>
        <AutoFixButton autoFixPrUrl={autoFixPrUrl} />
