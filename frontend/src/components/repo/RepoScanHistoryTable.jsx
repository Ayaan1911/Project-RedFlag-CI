...
export default function RepoScanHistoryTable() {
  const navigate = useNavigate();
  const { repoId } = useParams();
  const data = MOCK_SCAN_HISTORY;
    ...
    <div className="glass-1 flex flex-col pt-6 pb-2 px-6 h-full">
      ...
      <div className="w-full overflow-x-auto custom-scrollbar flex-1">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr>
              {['PR', 'Vibe Score', 'AI Conf', 'Critical', 'High', 'Medium', 'Time'].map(h => (
                <th key={h} style={{ font: '500 11px var(--font-mono)', textTransform: 'uppercase', letterSpacing: '1px', color: '#4A5070', paddingBottom: '16px', fontWeight: 'normal' }}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {data.map((row, i) => {
              ...
              const prNum = row.pr.replace('#', '');
              
              const currentRepo = repoId || 'demo';

              return (
                <tr 
                  key={row.pr} 
                  className="table-body-row group"
                  onClick={() => navigate(`/repo/${currentRepo}/scan/${prNum}`)}
                  style={active ? {
                ...
                >
                  ...
                  <td style={{ padding: '12px 8px' }}>
                    <div className="flex flex-col gap-[6px]">
                      <div>
                        <span style={{ font: '700 14px var(--font-display)', color: sColor }}>{row.score}</span>
                      ...
                      <div style={{ width: '60px', height: '4px', background: 'rgba(255,255,255,0.05)', borderRadius: '999px', overflow: 'hidden' }}>
                        <div style={{ width: `${row.score}%`, height: '100%', background: sColor }} />
                      </div>
                    </div>
                  </td>
      ...
      <div className="flex justify-between items-center mt-3 pt-4 border-t border-[rgba(255,255,255,0.05)]">
        <span style={{ font: '400 11px var(--font-body)', color: '#4A5070' }}>Showing 1-5 of 42 scans</span>
