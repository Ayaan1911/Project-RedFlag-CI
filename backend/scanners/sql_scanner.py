            ...
            for rule_name, pattern, severity, desc_template, fix_template in SQL_INJECTION_RULES:
                for i, line in enumerate(lines, 1):
                    if re.search(pattern, line, re.IGNORECASE):
                        if rule_name == "Raw ORM Query" and _is_parameterized_query(line):
                            continue
...
def _is_parameterized_query(line: str) -> bool:
    """Skip findings for execute/query/raw calls that already pass bound params."""
    normalized = re.sub(r"\s+", " ", line)
    has_placeholder = any(token in normalized for token in ("?", "%s", ":1", "$1"))
    has_bound_params = bool(re.search(r"(?:raw|execute|query)\s*\([^,]+,\s*[\(\[\{]", normalized, re.IGNORECASE))
