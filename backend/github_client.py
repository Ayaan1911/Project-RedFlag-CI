...
async def _get_repo_metadata(repo_full_name: str, client: httpx.AsyncClient, headers: dict) -> dict:
    resp = await client.get(
...
async def get_repo_default_branch(repo_full_name: str) -> str:
    """Return the repository default branch name."""
    headers = await _get_headers(repo_full_name)
    async with httpx.AsyncClient(timeout=15.0) as client:
        repo = await _get_repo_metadata(repo_full_name, client, headers)
        return repo["default_branch"]


async def get_branch_head_sha(repo_full_name: str, branch: str) -> str:
...
async def get_file_contents(repo_full_name: str, filepath: str, ref: str | None = None) -> dict | None:
    """Fetch file contents and sha from the GitHub Contents API."""
    headers = await _get_headers(repo_full_name)
    async with httpx.AsyncClient(timeout=15.0) as client:
        resp = await client.get(
        ...
        return {
            ...
            "path": payload.get("path", filepath),
        }
...
async def get_pr_diff(repo_full_name: str, pr_number: int) -> list[dict]:
    ...
    Returns: [{'filename': str, 'content': str, 'patch': str, 'sha': str | None}]
    """
    headers = await _get_headers(repo_full_name)
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        for f in pr_files:
            ...
            # 2. Fetch full file content + sha via contents API
            content = ""
            try:
                content_payload = await get_file_contents(repo_full_name, filename)
                if content_payload:
                    content = content_payload.get("content", "")
                    file_sha = content_payload.get("sha")
            except Exception as e:
                logger.warning("Could not fetch contents API metadata for %s: %s", filename, e)

            # 3. Fallback to raw URL if contents API did not return content
            if raw_url and not content:
                try:
